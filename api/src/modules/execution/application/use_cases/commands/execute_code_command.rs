    ...
    C: CodeRunner,
{
    pub fn new(submission_repository: SubmissionRepository, code_runner: C) -> Self {
        Self { submission_repository, code_runner }
    }

    pub async fn handle(&self, command: ExecuteCodeCommand) -> Result<RunResult, Box<dyn std::error::Error>> {
        let request = RunRequest{
            language: command.language.clone(),
            code: command.code.clone()
        };
        let result = self.code_runner.run(request).await?;
        let submission = self.submission_repository.create_submission(
            command.user_id,
            command.exercise_id,
            &command.code,
            &command.language
        ...
        self.submission_repository.update_status(
            submission.id,
            final_status,
            Some(result.duration_ms as i32)
        ).await?;
